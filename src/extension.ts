...
class GameDetailViewProvider implements vscode.WebviewViewProvider {
    ...
    private _getGenericHtml(game: GameMetadata, loading: boolean) {
        ...
        const tags = (game.extension.tags || []).map((t: string) => `<span style="background: #333; padding: 2px 6px; border-radius: 4px; font-size: 0.8em; margin-right: 4px; display: inline-block; margin-bottom: 4px;">${t}</span>`).join('');
...
class Pico8CartPanel {
    ...
    public static createOrShow(extensionUri: vscode.Uri, game: GameMetadata, cartData: CartData) {
    ...
    private _getHtmlForWebview(game: GameMetadata, cartData: CartData) {
...
export function activate(context: vscode.ExtensionContext) {
    ...
    // Select Game
    vscode.commands.registerCommand('pico8ide.selectGame', (game: GameMetadata) => {
